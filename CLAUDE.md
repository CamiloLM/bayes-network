...
- Design system: every color, font and size comes from `escenas/style.py`; repeated visuals (`RedBayesiana` and `red_alarma()` / `red_aspersor()` with node states and `renombrar()`, CPT tables, calculation blocks, highlighted results, section titles, lists, phone) come from `escenas/components.py`. Build text with `components.texto()` / `titulo()`, not `Text` directly: they work around Manim's bad letter spacing at small font sizes. A row of words must be one `texto(...)` (separate Texts don't share a baseline); reveal its pieces with `components.partes()`. Centered multi-line text is `parrafo("a\nb")`.
- Scene idioms (`EscenaNarrada`): sync to the narration with `self.en(bloque, "frase del TEXTO_TTS")`; `limpiar()` clears the screen; `cambiar(a, b)` swaps two mobjects — don't use `FadeTransform`, it leaves an orphan copy in the scene. Animating a submobject (`red.nodos[x].animate...`) splits its parent into loose pieces in the scene, so animate whole groups (`red.estados(...)`, `red.renombrar(...)`) or call `consolidar(grupo)` afterwards.
- `renderizar.py -j N` can make two scenes collide in Manim's LaTeX/text caches; failed scenes are retried serially.
